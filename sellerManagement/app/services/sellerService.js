...
module.exports.getSellerDetailsService = async (requestBody) => {
  try {
    let id = requestBody.seller_id;

    let sellerObj = await Seller.findOne({ _id: id });

    if(!sellerObj){
      throw new BadRequestException("No any seller profiles")
    }
    return {
      msg: "success",
      data: sellerObj,
